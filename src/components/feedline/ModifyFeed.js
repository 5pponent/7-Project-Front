...
export default React.memo(function ModifyFeed(props) {
  ...
  useEffect(() => {
    ...
    files.map(item => (
      fetch(item.source)
        .then(res => setFile([...file, new Blob(res.data)]))
    ))
  }, []);
  ...
  const handleDeleteOriginFeedImage = (num) => {
    const newFeedImage = originImage.filter((item, index) => index !== num);
    setOriginImage(newFeedImage);
  };
  ...
  const handleChangeOriginDescription = (e, num) => {
    const newDescription = originImage.map((item, index) =>
      index === num ? {...item, description: e.target.value} : item);
    setOriginImage(newDescription);
  };
  const handleAddFeedImage = (e) => {
    let newImage = feed.image;
  ...
  const handleModifyFeed = async (feedId) => {
    if (validate()) {
      console.log(originImage.concat(feed.image))
      dispatch({type: 'OpenLoading', payload: '피드를 작성중입니다..'});
      const feedForm = new FormData();
      feedForm.append('content', feed.content);
      originImage.concat(feed.image).map(item => feedForm.append('descriptions', item.description));
      file.map(item => feedForm.append('images', item));
      feed.image.map(item => feedForm.append('images', item.file));
      feedForm.append('showScope', scope);
      console.log(feedForm.get('descriptions'))
      console.log(feedForm.get('images'))
                  ...
                  <CardContent style={{height: '100%'}}>
                    <Grid container spacing={1}
                      ...
                      <Grid item style={{padding: 0}} sx={{flex: 0.2, width: '100%'}}>
                        <TextField placeholder='설명' value={item.description}
